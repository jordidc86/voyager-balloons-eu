...
class ConfigTests(unittest.TestCase):
    def test_production_config_has_required_scope(self) -> None:
        ...
        self.assertGreaterEqual(len(config["strategic_pages"]), 9)
        self.assertEqual(config["tracking"]["store_domain"], "tienda.voyagerballoons.eu")
        self.assertGreaterEqual(len(config["commerce_probes"]), 5)
        ...
        self.assertEqual(set(config["brands"]), {"voyager", "ao_sabor_do_vento"})
        ...
        self.assertTrue(all(row["brand_id"] in config["brands"] for row in keywords))
        self.assertTrue(all(row["role"] in {"primary", "cross_sell"} for row in keywords))
        braganca_pt = next(row for row in keywords if row["keyword"] == "passeio de balao braganca")
        self.assertEqual(braganca_pt["brand_id"], "ao_sabor_do_vento")
