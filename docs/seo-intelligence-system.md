...
- **P1:** 4XX, canonical incorrecto, caída significativa de clics o conversiones, posición estratégica perdida, backlink valioso ausente en dos controles consecutivos.
...
- Tests locales: 132/132 correctos, incluyendo configuración multservicio de Railway, equivalencia de redirecciones, confirmación temporal de alertas, separación de tiendas por dominio y diagnóstico accionable de PageSpeed.
...
- El 29 de agosto se retiró WordPress del flujo público y se activó `tienda.voyagerballoons.eu`. GA4 separa desde entonces la tienda directa actual, la campaña `google_business_profile` y el histórico de `shop.voyagerballoons.eu`; ningún evento o ingreso del respaldo WordPress puede contaminar el diagnóstico del embudo nuevo.
...
- PageSpeed completo revalidado: 20/20 pruebas correctas a nivel de proveedor. CrUX conserva una ventana móvil de 28 días con TTFB p75 de 3,13 s, FCP de 3,50 s y LCP de 3,52 s; INP y CLS son correctos. En la medición actual con caché caliente el documento responde en unos 10 ms. El plugin reversible `Voyager Shop Fixes` convierte los controles de cantidad de Astra en controles rastreables y accesibles, evita que WP Rocket retrase esa corrección, conserva la galería y el visor ampliado sin descargar la imagen invisible de zoom, y retira CSS opcional no utilizado solo en fichas de producto. Mediciones móviles frescas: SEO 100, buenas prácticas 100, accesibilidad 97 y rendimiento de laboratorio 79 en Comfort y 80 en Bragança; los cinco flujos de compra y las nueve comprobaciones de tracking siguen correctos.
- El 21 de julio de 2026 se añadió el puente de un solo uso `Voyager GA4 add_to_cart bridge`. Conserva el producto en la sesión cuando WooCommerce fuerza la redirección al carrito y emite exclusivamente `add_to_cart` mediante Site Kit en la página siguiente. No interviene en `purchase`; el monitor comprueba diariamente que el puente exista y que WP Rocket no lo retrase.
- La intención española de regalo se consolidó en `/regalar-vuelo-en-globo-segovia`: la landing genérica redirige directamente por 301, el artículo mantiene una intención informativa diferenciada y los enlaces comerciales internos apuntan a una única URL. Las caídas de posición solo escalan a P1 cuando Search Console confirma al menos 10 impresiones en siete días para la consulta exacta; sin esa evidencia permanecen como P2 de seguimiento.
